...
test('couples rest depth to visible hard shadow', () => {
  assert.deepEqual(resolvePhysicalOffset('rest', 4), {
    state: 'rest',
    depth: 4,
    translateX: 0,
    translateY: 0,
...
test('press translates by full depth and collapses shadow', () => {
  ...
  assert.equal(pressed.translateX, 8);
  assert.equal(pressed.translateY, 8);
  assert.equal(pressed.shadowX, 0);
  assert.equal(pressed.shadowY, 0);
});
...
test('clamps unsupported depth and disables motion for disabled state', () => {
  ...
  assert.equal(disabled.transitionMs, 0);
  assert.equal(disabled.opacity, 0.55);
});
...
test('reduced motion preserves state without transition duration', () => {
  ...
  assert.equal(selected.transitionMs, 0);
  assert.equal(selected.depth, 4);
});
